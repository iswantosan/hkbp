...
class _HomeState extends State<Home> with SingleTickerProviderStateMixin {
  ...
  Future<void> _fetchKidungJemaatList() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Cari semua elemen yang berisi "KJ" dan nomor
        // Format: "KJ 1 - Haleluya, Pujilah"
        final bodyText = document.body?.text ?? '';
        
        // Pattern untuk mencari semua "KJ [nomor] - [judul]"
        final kjPattern = RegExp(r'KJ\s+(\d+)\s*[-–]\s*(.+?)(?=\n|KJ\s+\d+|$)', multiLine: true);
        final matches = kjPattern.allMatches(bodyText);
        ...
        for (final match in matches) {
          ...
          if (nomor > 0 && title.isNotEmpty) {
            final url = 'https://alkitab.mobi/kidung/kj/$nomor/';
            debugPrint('Kidung Jemaat - Nomor: $nomor, Title: $title, URL: $url');
        ...
        // Jika tidak ada hasil dari pattern, coba dari link
        if (kidungJemaatList.isEmpty) {
          final links = document.querySelectorAll('a');
          ...
          for (final link in links) {
            ...
            // Pattern: "KJ 1 - Haleluya, Pujilah" atau "KJ 1"
            final kjMatch = RegExp(r'KJ\s+(\d+)\s*[-–]\s*(.+)').firstMatch(text);
            if (kjMatch != null) {
              final nomor = int.tryParse(kjMatch.group(1) ?? '') ?? 0;
              final title = kjMatch.group(2)?.trim() ?? '';
              
              if (nomor > 0 && title.isNotEmpty) {
                final url = href ?? 'https://alkitab.mobi/kidung/kj/$nomor/';
                kidungJemaatList.add(KidungJemaat(
                  title: title,
                  nomor: nomor,
                  url: url,
              ...
              // Alternatif pattern: hanya "KJ 1" tanpa judul
              final kjSimpleMatch = RegExp(r'KJ\s+(\d+)').firstMatch(text);
              if (kjSimpleMatch != null && href != null && href.contains('/kj/')) {
                final nomor = int.tryParse(kjSimpleMatch.group(1) ?? '') ?? 0;
                if (nomor > 0) {
                  final url = href.startsWith('http') ? href : 'https://alkitab.mobi$href';
                  debugPrint('Kidung Jemaat (from link) - Nomor: $nomor, URL: $url');
                  kidungJemaatList.add(KidungJemaat(
                    title: 'Kidung Jemaat $nomor',
                    nomor: nomor,
                    url: url,
                  ));
                }
        ...
        // Sort berdasarkan nomor dan hapus duplikat
        kidungJemaatList.sort((a, b) => a.nomor.compareTo(b.nomor));
        // Hapus duplikat berdasarkan nomor
        final seen = <int>{};
        kidungJemaatList.removeWhere((item) => !seen.add(item.nomor));
  ...
  Future<void> _fetchKidungJemaatLyrics(String url) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Cari konten utama lagu - coba berbagai selector
        ...
        debugPrint('Content found: ${content != null}');
        
        String lyrics = '';
        if (content != null) {
          // Hapus elemen share button dan social media
          content.querySelectorAll('.sharedaddy, .sd-block, .jetpack-share, .social-share, [class*="share"], [class*="facebook"], [class*="twitter"], [id*="share"], nav, header, footer').forEach((el) => el.remove());
          
          // Coba ambil dari list items terlebih dahulu (biasanya lirik lagu dalam format list)
          final listItems = content.querySelectorAll('li');
          debugPrint('List items found: ${listItems.length}');
          ...
          // Jika tidak ada list items, coba ambil dari paragraf
          if (lyrics.isEmpty) {
            final paragraphs = content.querySelectorAll('p');
            ...
            for (final p in paragraphs) {
              ...
              if (text.isNotEmpty && 
                  ...
                  !text.toLowerCase().contains('like') &&
                  !text.toLowerCase().contains('follow') &&
                  !text.toLowerCase().contains('alkitab.mobi') &&
          ...
          // Jika masih kosong, coba ambil dari div dengan class tertentu
          if (lyrics.isEmpty) {
            final divs = content.querySelectorAll('div');
            debugPrint('Divs found: ${divs.length}');
            
            for (final div in divs) {
              final text = div.text.trim();
              // Skip jika terlalu pendek atau terlalu panjang (kemungkinan bukan lirik)
              if (text.length > 20 && text.length < 5000 &&
                  ...
                  !text.toLowerCase().contains('alkitab.mobi') &&
                  !text.toLowerCase().contains('sabda')) {
                // Cek apakah ini kemungkinan lirik (ada beberapa baris)
          ...
          debugPrint('Lyrics preview: ${lyrics.length > 100 ? lyrics.substring(0, 100) : lyrics}');
        } else {
        ...
        if (lyrics.isNotEmpty) {
          ...
          debugPrint('Lyrics is empty, setting error message');
          setState(() {
        ...
        setState(() {
          _kidungJemaatLyrics[url] = 'Gagal memuat lirik (HTTP ${response.statusCode})\nURL: $url';
          _loadingKidungJemaatLyrics[url] = false;
        });
      }
    } catch (e) {
    ...
    debugPrint('=== END FETCH KIDUNG JEMAT LYRICS ===');
  }
              ...
              : _kidungJemaatError != null
                  ...
                  : _filteredKidungJemaatList.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            // Jika index adalah item terakhir, tampilkan info sumber dan copyright
  ...
  /// Fetch daftar Buku Nyanyian dari alkitab.app
  Future<void> _fetchBukuNyanyianList() async {
    ...
    try {
      // Coba cari halaman daftar terlebih dahulu
      // Pattern: https://alkitab.app/BN/ atau https://alkitab.app/buku-nyanyian/
      final List<String> possibleListUrls = [
        'https://alkitab.app/BN/',
        'https://alkitab.app/buku-nyanyian/',
        'https://alkitab.app/BN',
      ];
      
      final List<BukuNyanyian> bukuNyanyianList = [];
      bool foundListPage = false;
      
      // Coba akses halaman daftar
      for (final listUrl in possibleListUrls) {
        try {
          final response = await http.get(
            Uri.parse(listUrl),
            headers: {
          ...
          if (response.statusCode == 200) {
            ...
            // Cari semua link yang berisi "BN" dan nomor
            final links = document.querySelectorAll('a');
            ...
            for (final link in links) {
              ...
              // Pattern: "BN 23" atau "BN 23 - Judul Lagu"
              final bnMatch = RegExp(r'BN\s+(\d+)(?:\s*[-–]\s*(.+))?').firstMatch(text);
              if (bnMatch != null) {
                final nomor = int.tryParse(bnMatch.group(1) ?? '') ?? 0;
                final title = bnMatch.group(2)?.trim() ?? 'Buku Nyanyian $nomor';
                
                if (nomor > 0) {
                  final url = href != null 
                      ? (href.startsWith('http') ? href : 'https://alkitab.app$href')
                      : 'https://alkitab.app/BN/$nomor';
                  
                  // Pastikan URL valid dan belum ada di list
                  if (!bukuNyanyianList.any((item) => item.nomor == nomor)) {
            ...
            if (bukuNyanyianList.isNotEmpty) {
              foundListPage = true;
              break;
            }
          }
      ...
      // Jika tidak ada halaman daftar, coba fetch beberapa nomor untuk mendapatkan daftar
      // Kita akan coba nomor 1-200 (lebih efisien dari 500)
      if (!foundListPage && bukuNyanyianList.isEmpty) {
        int consecutiveErrors = 0;
        for (int i = 1; i <= 200; i++) {
          final url = 'https://alkitab.app/BN/$i';
          try {
            final response = await http.get(
              Uri.parse(url),
              headers: {
            ...
            if (response.statusCode == 200) {
              ...
              // Skip jika halaman error atau tidak ada konten
              if (bodyText.toLowerCase().contains('not found') ||
                  bodyText.toLowerCase().contains('404') ||
                  bodyText.length < 100) {
                consecutiveErrors++;
                if (consecutiveErrors >= 5) {
                  // Jika 5 nomor berturut-turut error, kemungkinan sudah sampai akhir
                  break;
                }
                continue;
              }
              ...
              // Coba ambil dari title tag
              final titleTag = document.querySelector('title');
              if (titleTag != null) {
                final titleText = titleTag.text.trim();
                final titleMatch = RegExp(r'BN\s+\d+\s*[-–]\s*(.+?)(?:\s*\||$)').firstMatch(titleText);
                if (titleMatch != null) {
                  title = titleMatch.group(1)?.trim();
                }
              }
              
              // Jika tidak ada di title, coba dari h1
              if (title == null || title.isEmpty) {
                final h1 = document.querySelector('h1');
                if (h1 != null) {
                  final h1Text = h1.text.trim();
                  final h1Match = RegExp(r'BN\s+\d+\s*[-–]\s*(.+)').firstMatch(h1Text);
                  if (h1Match != null) {
                    title = h1Match.group(1)?.trim();
                  } else if (h1Text.isNotEmpty && !h1Text.toLowerCase().contains('not found')) {
                    title = h1Text;
                  }
                }
              }
              
              // Jika masih tidak ada, coba dari konten
              if (title == null || title.isEmpty) {
                ...
                if (content != null) {
                  ...
                  final contentMatch = RegExp(r'BN\s+\d+\s*[-–]\s*(.+?)(?:\n|$)').firstMatch(contentText);
                  if (contentMatch != null) {
                    title = contentMatch.group(1)?.trim();
                  }
                }
              }
              
              // Jika ada judul, tambahkan ke list
          ...
          // Batasi jumlah request untuk menghindari timeout
          if (i % 10 == 0) {
  ...
  /// Fetch detail lirik Buku Nyanyian
  Future<void> _fetchBukuNyanyianLyrics(String url) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (content != null) {
          // Hapus elemen share button dan social media
          content.querySelectorAll('.sharedaddy, .sd-block, .jetpack-share, .social-share, [class*="share"], [class*="facebook"], [class*="twitter"], [id*="share"]').forEach((el) => el.remove());
          
          // Ambil semua paragraf atau list items
          final paragraphs = content.querySelectorAll('p');
          final listItems = content.querySelectorAll('li');
                  ...
                  : _filteredBukuNyanyianList.isEmpty
                      ? Center(
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 16),
                              Text(
                                _searchBukuNyanyianController.text.isNotEmpty
                                    ? 'Tidak ada hasil pencarian'
                                    : 'Belum ada data. Fitur ini akan segera tersedia.',
                                style: const TextStyle(color: Colors.grey),
                              ),
                            ],
                          ),
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            if (index == _filteredBukuNyanyianList.length) {
                              return Container(
                                ...
                                child: Column(
                                  children: [
                                    Text(
                                      'Sumber data: https://alkitab.app/BN/',
                                      textAlign: TextAlign.center,
                                      style: TextStyle(
                                        fontSize: 11,
                                        color: Colors.grey[500],
                            ...
                            return _buildBukuNyanyianItem(lagu);
                          },
                        ),
        ),
      ],
  ...
  Widget _buildBukuNyanyianItem(BukuNyanyian lagu) {
    final hasLyrics = _bukuNyanyianLyrics.containsKey(lagu.url);
